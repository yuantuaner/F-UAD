...
def plot_accuracies(accuracy_list, val_loss_list, folder):
	os.makedirs(f'plots/{folder}/', exist_ok=True)
	trainAcc = [i[0] for i in accuracy_list]
	lrs = [i[1] for i in accuracy_list]
	valACC = val_loss_list
	...
	plt.twinx()
	plt.plot(range(len(valACC)), valACC, label='Average valing Loss', color='b', linewidth='-', linestyle='--', marker='.')	
	plt.savefig(f'plots/{folder}')
	plt.clf()
