...
def dist(t, q):      # 计算了两个等长度序列 t 和 q 之间的欧氏距离
	m = q.shape[0]
...
def getsub(t, L, i):       # 从时间序列 t 中提取了从索引 i 开始，长度为 L 的子序列
	return t[i:i+L]
...
# MERLIN
def merlin(t, minL, maxL):         # 调用的参数是 a, 1, 10
	r = 2 * np.sqrt(minL)
	dminL = - maxint; DFinal = []     # maxint = 200000 , 生成一个DFinal列表
	while dminL < 0:
		C = csa(t, minL, r)
		D = drag(C, t, minL, r)
		r = r / 2
	...
	distances = [-maxint] * 4
	print('phase 1')
	for i in range(minL, min(minL+4, maxL)):   # 最多遍历从minL开始的4个元素，但不超过maxL
		di = distances[i - minL]
		dim1 = rstart if i == minL else distances[i - minL - 1]
		r = 0.99 * dim1
		while di < 0:
			C = csa(t, i, r)
			D = drag(C, t, i, r)              # 找到最小距离
			if D: 
